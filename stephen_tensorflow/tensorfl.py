...
#want to init weights to random values
    ...
    #y = mr.integers(fp_min, fp_max, [8,64]).astype(np.float32)/(scale)
    #z = mr.integers(fp_min, fp_max, [64,2048]).astype(np.float32)/(scale)
#this made redundant? #w = mr.integers(fp_min, fp_max, [16,8]).astype(np.float32)/(scale)


    x,y,z = set_decoder_f5_input2048()
    #x = convert_weights(x)
    #y = convert_weights(y)
    #z = convert_weights(z)
    #w = convert_weights(w)
...
#optimizer
    opt = tf.keras.optimizers.Adam(learning_rate=5e-7)#was -7//// #5e-3)
...
#autoencoder.compile(optimizer=opt, loss=losses.CategoricalHinge())
    ...
    history = autoencoder.fit(X,X, epochs=150000)
...
#f = open('./weights_f5_input2048.txt')
#print(autoencoder.layers[1].weights)
#f.close()
